...
deb-src http://ftp.debian.org/debian buster-backports main contrib non-free" > /etc/apt/sources.list
	...
	sudo -u $MAINUSER cp defaults.vim ~/.vimrc
	apt-get purge -y vim
	sudo -u $MAINUSER echo "
alias q='exit'
alias r='ranger'
alias v='vim'
set -o vi" >> ~/.bashrc
...
set clipboard+=unnamedplus" >> ~/.vimrc
	sudo -u $MAINUSER mkdir ~/.config/nvim ~/.vim ~/.vim/plugged ~/.config/ranger
	sudo -u $MAINUSER echo "
set runtimepath+=~/.vim,~/.vim/after
set packpath+=~/.vim
source ~/.vimrc" > ~/.config/nvim/init.vim
	sudo -u $MAINUSER echo "Now, please press ":" key and type PlugInstall, then wait about 1-2 minutes. After everything is installed, press <ESC>, to enter normal mod, and press <ZQ> to exit Vim" | nvim
	cd "`eval echo ~$MAINUSER`/.vim/plugged/YouCompleteMe"
	sudo -u $MAINUSER python3 install.py --clang-completer --ts-completer 
	sudo -u $MAINUSER echo "
