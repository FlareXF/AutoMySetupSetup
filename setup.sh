...
set -o vi" >> ~/.bashrc
	...
	cp "`eval echo ~$USER`/.bashrc" /root/.bashrc 
...
set clipboard+=unnamedplus" >> ~/.vimrc
	sudo -u $USER mkdir ~/.config/nvim ~/.vim ~/.vim/plugged ~/.config/ranger
	sudo -u $USER echo "
set runtimepath+=~/.vim,~/.vim/after
set packpath+=~/.vim
source ~/.vimrc" > ~/.config/nvim/init.vim
	sudo -u $USER echo "Now, please press ":" key and type PlugInstall, then wait about 1-2 minutes. After everything is installed, press <ESC>, to enter normal mod, and press <ZQ> to exit Vim" | nvim
	cd "`eval echo ~$USER`/.vim/plugged/YouCompleteMe"
